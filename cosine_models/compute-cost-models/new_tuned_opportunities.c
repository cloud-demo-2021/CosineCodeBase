...
int main(int argc, char* argv[]) {
    ...
    read_percentage = 10; // point lookups
    write_percentage = 25; // inserts 
    rmw_percentage = 5; // read-modify writes 
    blind_update_percentage = 30; // blind updates
    short_scan_percentage = 0;
    long_scan_percentage = 10; // range queries
    long_scan_empty_percentage = 20; // empty range queries
    ...
    if(strcmp(argv[2], "WT") == 0)
    {
        ...
        avg_read_cost = avg_read_cost * B_TREE_CACHE_DISCOUNT_FACTOR/0.5; // Discounting factor due to caching in Btrees
    ...
    {
        // run with ./run 0 rocks 32 1000000 or ./run 0 rocks 64 1000000
        T = 10; 
        K = 1;
        Z = 1;
        Y = 0;
        ...
        // Adjust design
        int bits;
        for (bits=9; bits>1 && M_B < 0; bits--)
        {
            M_BF = data * (double)bits/8.0; 
            M_F = M_BF + M_FP;
            M_B = M - M_F; 
        }

        if(M_B < 0 || bits == 0) // no appropriate tuning found
        {
            printf("no appropriate tuning found\n");
            printf("Atleast %fGB memory needed\n", M_F/(1024*1024*1024));
            return 0;
        }
        
        getNoOfLevelsAvgCase(&L, M_B, T, data);
        printf("M_B:%f, M_F:%f M_BF:%d bits/entry L:%d\n", M_B/(1024*1024*1024), M_F/(1024*1024*1024), bits, L);
    ...
    {
        // run with ./verify 0 FASTER_H 32 or ./verify 0 FASTER_H 64 
        ...
        // Adjust design
        int tuned_scale_factor = scale_factor;
        for (tuned_scale_factor=1000; tuned_scale_factor<=10000 && M_B < 0; tuned_scale_factor=tuned_scale_factor+100)
        {
            M_F = ((double)data/tuned_scale_factor)*(F)*(1.0 + (1.0/B)); 
            M_B = M - M_F;
        }

        if(M_B < 0 || tuned_scale_factor < 2) // no appropriate tuning found
        {
            printf("no appropriate tuning found\n");
            printf("Atleast %fGB memory needed\n", M_F/(1024*1024*1024));
            return 0;
        ...
        Y = 0;
        L = 1;
        printf("M_F: %f, M_B: %f, T: %d scale_factor:%d\n", M_F/(1024*1024*1024), M_B/(1024*1024*1024), T, tuned_scale_factor);
