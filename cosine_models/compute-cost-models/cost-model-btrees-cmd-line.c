...
void computeCosts(double M, double M_B, double M_BC, double M_BF, int T, int K, int Z, double data)
{
...
int main(int argc, char* argv[]) {
	if (argc < 2) {
      fprintf(stderr, "Usage:<workload_type>, <T>, <K>, <Z>, <M>, <MB>, <MBF>, <MBC>, <U or U_1,U_2,p_put,p_get>");
      exit(1);
    }
    workload_type = atoi(argv[1]);
    int expected_num = 10;
    if (workload_type == 1) expected_num = 13;
    if (argc != expected_num) {
      fprintf(stderr, "Usage:<workload_type>, <U>, <T>, <K>, <Z>, <M>, <MB>, <MBF>, <MBC>");
      exit(1);
    }	
	...
	int K = atoi(argv[3]);
	int Z = atoi(argv[4]);
	double M = atof(argv[5]) * 1000000000;
	double M_B = atof(argv[6]) * 1000000000;
	double M_BF = atof(argv[7]) * 1000000000;
	double M_BC = atof(argv[8]) * 1000000000;
	...
	B = 128; // set environment variable
	printf("B:%d\n", B);
	double data = 10000000;
	computeCosts(M, M_B, M_BC, M_BF, T, K, Z, data);
