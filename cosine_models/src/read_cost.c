...
double propLastlevelFull(int T, int K, int Z, int L, int Y, double M_B, double data)
{

	double last_level;
	last_level = M_B*pow(T,L)/(E);
...
double getAlpha_i(int type, double M_B, double M_BC, int T, int K, int Z, int L, int Y, int i, double data)
{
	...
	if (i == L) {
		...
		{
			double prop_last_level_full = propLastlevelFull(T, K, Z, L, Y, M_B, data);
...
void getcq(int type, int T, int K, int Z, int L, int Y, double M_B, double M_BC, double* c, double* q, double data)
{
	*q = 1.0;
	for(int i=1;i<=L-Y-1;i++)
	{
		*q = (*q) * pow((1.0 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, i, data)), K);
	}
	int hot_level_boundary = L - Y > 1 ? L - Y : 1;
	for(int i=hot_level_boundary;i<=L;i++)
	{
		*q = (*q) * pow((1.0 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, i, data)), Z);
	}
	// double alpha_0 = getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, 0, data);
	*c = (1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, -1, data)) * (1 - (*q))*(1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, 0, data));
	*q = 1 - (*q)*(1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, 0, data));
	//printf("c: %f, q: %f, alpha_0: %f\n", (*c), (*q), alpha_0);
}

double getC_ri(int type, int r, int i, double M_B, double M_BC, int T, int K, int Z, int L, int Y, double data)
{
	double term1 = 1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, 0, data);
	term1 *= (1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, -1, data));
	//printf("Term1:%f\n", term1);
	double term2 = 1.0;
	for(int h=1;h<i;h++)
	{
		term2 = term2 * pow((1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, h, data)), K);
	}
	double term3 = pow((1.0 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, i, data)), r);
	double term4 = 1;
	for(int h = i+1;h<=L-Y-1;h++)
	{
		term4 = term4 * pow((1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, h, data)), K);
	}
	for(int h = L-Y;h<=L;h++)
	{
		if (h != 0) {
			term4 = term4 * pow((1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, h, data)), Z);
		}
	}
	term4 = term4 * pow((1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, i, data)), K-r);
	term4 = 1 - term4;
	return term1*term2*term3*term4;
}

double getD_ri(int type, int r, int i, double M_B, double M_BC, int T, int K, int Z, int L, int Y, double data)
{
	double term1 = 1.0 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, 0, data);
	term1 *= (1.0 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, -1, data));
	// printf("term1:%f\n", term1);
	...
	{
		if (h != 0) {
			term2 = term2 * pow((1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, h, data)), Z);
		}
	}
	double term3 = pow((1.0 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, i, data)), r);
	double term4 = 1.0;
	...
	term4 = term4 * pow((1 - getAlpha_i(type, M_B, M_BC, T, K, Z, L, Y, i, data)), Z-r);
	term4 = 1 - term4;
	return term1*term2*term3*term4;
}

double aggregateAvgCase(int type, double* FPR_sum, int T, int K, int Z, int L, int Y, double M, double M_B, double M_BC, double M_F, double M_BF, double data) {
	double term1 = 0.0, term2 = 0.0, term3= 0.0;
	double term2_2 = 0.0, term3_2 =0.0;
	double c, q;
	double p_i;
	getcq(type, T, K, Z, L, Y, M_B, M_BC, &c, &q, data);
	term1 = c/q;
	*FPR_sum = getFPRSum(M_BF, T, K, Z, L, Y, data);
	for(int i = 1;i<=L-Y-1;i++)
	{
		...
		for(int r = 1;r<=K;r++)
		{
			term2_2 = term2_2 + getC_ri(type, r, i, M_B, M_BC, T, K, Z, L, Y, data)/q;
		}
	...
	for(int i = hot_level_boundary;i<=L;i++)
	{
		...
		for(int r = 1;r<=Z;r++)
		{
			term3_2 = term3_2 + (getD_ri(type, r, i, M_B, M_BC, T, K, Z, L, Y, data)/q);
			// printf("q:%f\n", q);
...
double aggregateAvgCaseCPU(int type, int T, int K, int Z, int L, int Y, double M, double M_B, double M_BC, double M_F, double M_BF) {
	double term1 = 0.0, term2 = 0.0, term3= 0.0;
	double term2_2 = 0.0;
	double c, q;
	getcq(type, T, K, Z, L, Y, M_B, M_BC, &c, &q, N); // N should be data actually - take care of it before using it
	term1 = c/q;
	for(int i = 1;i<=L-Y-1;i++)
	{
		term2_2 = 0.0;
		for(int r = 1;r<=K;r++)
		{
			term2_2 = term2_2 + getC_ri(type, r, i, M_B, M_BC, T, K, Z, L, Y, N)/q; // N should be data actually - take care of it before using it
		}
		term2 = term2 + term2_2;
	}
	...
	for(int r = 1;r<=Z;r++)
	{
		term3 = term3 + getD_ri(type, r, L - Y, M_B, M_BC, T, K, Z, L, Y, N)/q; // N should be data actually - take care of it before using it
	}
	return term1 + term2 + term3;
}
...
void analyzeReadCostAvgCase(double* avg_read_cost, double* FPR_sum, int T, int K, int Z, int L, int Y, double M, double M_B, double M_BC, double M_F, double M_BF, double data)
{
	if(Z == 0) // LSH-table append-only
	{
		...
		double term1;
		double c, q;
		double alpha_0 = getAlpha_i(workload_type, M_B, M_BC, T, K, Z, L, Y, 0, data);
		//printf("alpha_0: %f\n", alpha_0);
		q = pow((1.0 - getAlpha_i(workload_type, M_B, M_BC, T, K, Z, L, Y, 1, data)), K);
		c = (1 - q)*(1 - getAlpha_i(workload_type, M_B, M_BC, T, K, Z, L, Y, 0, data));
		q = 1 - q*(1 - getAlpha_i(workload_type, M_B, M_BC, T, K, Z, L, Y, 0, data)); 
		term1 = c;
	...
	else if (Z == -1) // LSH-table hybrid logs
	{
		double term1;
		double c, q;
		double alpha_mutable = getAlpha_i(workload_type, 0.9*M_B, M_BC, T, K, Z, L, Y, 0, data);
		double alpha_read_only = getAlpha_i(workload_type, 0.1*M_B, M_BC, T, K, Z, L, Y, 0, data);	
		double alpha_0 = 1 - ((1 - alpha_mutable) * (1 - alpha_read_only));
		q = pow((1.0 - getAlpha_i(workload_type, M_B, M_BC, T, K, 1, L, Y, 1, data)), K);
		c = (1 - q)*(1 - alpha_0);
		q = 1 - q*(1 - alpha_0); 
		term1 = c;
	...
	// uniform
	if (workload_type == 0) {
		*avg_read_cost = aggregateAvgCase(0, FPR_sum, T, K, Z, L, Y, M, M_B, M_BC, M_F, M_BF, data);
		return;
	}

	// skew
	if (workload_type == 1) {
		double skew_part =  aggregateAvgCase(1, FPR_sum, T, K, Z, L, Y, M, M_B, M_BC, M_F, M_BF, data);
		double non_skew_part =  aggregateAvgCase(2, FPR_sum, T, K, Z, L, Y, M, M_B, M_BC, M_F, M_BF, data);
...
void analyzeReadCostAvgCaseCPU(double* avg_read_cost_cpu, int T, int K, int Z, int L, int Y, double M, double M_B, double M_BC, double M_F, double M_BF)
{
	// uniform
	if (workload_type == 0) {
		*avg_read_cost_cpu = aggregateAvgCaseCPU(0, T, K, Z, L, Y, M, M_B, M_BC, M_F, M_BF);
		return;
	}

	// skew
	if (workload_type == 1) {
		double skew_part =  aggregateAvgCaseCPU(1, T, K, Z, L, Y, M, M_B, M_BC, M_F, M_BF);
		double non_skew_part =  aggregateAvgCaseCPU(2, T, K, Z, L, Y, M, M_B, M_BC, M_F, M_BF);
